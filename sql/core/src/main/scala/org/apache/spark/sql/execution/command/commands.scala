...
case class ExecutedCommandExec(cmd: RunnableCommand, children: Seq[SparkPlan]) extends SparkPlan {
  ...
  override def requiredChildDistribution: Seq[Distribution] = cmd.requiredChildDistribution

  override def requiredChildOrdering: Seq[Seq[SortOrder]] = cmd.requiredChildOrdering
