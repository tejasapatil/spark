    ...
    val rdd = if (!relation.isPartitioned) {
      Utils.withDummyCallSite(sqlContext.sparkContext) {
        relation.tableMeta.bucketSpec match {
          case Some(bucketSpec) if sparkSession.sessionState.conf.bucketingEnabled =>
            hadoopReader.makeRDDForBucketedTable(hiveQlTable, bucketSpec)
          case _ => hadoopReader.makeRDDForTable(hiveQlTable)
        }
      }
    } else {
      Utils.withDummyCallSite(sqlContext.sparkContext) {
        val prunedPartitions = prunePartitions(rawPartitions)
        hadoopReader.makeRDDForPartitionedTable(prunedPartitions)
        /*
        relation.tableMeta.bucketSpec match {
          case Some(bucketSpec) if sparkSession.sessionState.conf.bucketingEnabled =>
            hadoopReader.makeRDDForBucketedPartitionedTable(
              hiveQlTable, prunedPartitions, bucketSpec)
          case _ =>
        }
        */
      }
